var resume = {
	...
	createCertificateUploader : function(csrfToken){
		...
		$("#certificateFile").fileinput({
			uploadUrl: '/resume/edit/certificates/upload?_csrf='+csrfToken,
		 	allowedFileExtensions : ['jpg', 'png'],
			maxFileCount: 1,
			showPreview:false
		});
	...
	moreProfiles : function(searchQuery) {
		...
		var url = '/resume/fragment/more?page=' + page;
	...
	hobbies : {
		...
		save : function() {
			...
			resume.post('/resume/edit/hobbies', {
				hobbies : hobbies,
				_csrf : _csrfToken
			});
		}
